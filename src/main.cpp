...
bool TextFitsInEdit(HWND edit, const std::wstring& text) {
    ...
    const int clientW = std::max(1, client.right - client.left);
    const int clientH = std::max(1, client.bottom - client.top);
...
LRESULT CALLBACK BoardProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        ...
        case WM_MOUSEMOVE: {
            ...
            if (g_dragEdge == DragEdge::Right) t.w = std::max(1, t.w + dx);
            if (g_dragEdge == DragEdge::Bottom) t.h = std::max(1, t.h + dy);
            if (g_dragEdge == DragEdge::Left) {
                t.x += dx;
                t.w -= dx;
                if (t.w < 1) {
                    t.w = 1;
                    t.x = g_originalTile.x + g_originalTile.w - 1;
                }
            }
            if (g_dragEdge == DragEdge::Top) {
                ...
                if (t.h < 1) {
                    t.h = 1;
                    t.y = g_originalTile.y + g_originalTile.h - 1;
                }
            }

            g_state.tiles[g_dragTile].x = std::max(0, t.x);
            g_state.tiles[g_dragTile].y = std::max(0, t.y);
